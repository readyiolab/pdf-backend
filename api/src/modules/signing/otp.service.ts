...
export const otpService = {
  async issue(
  ...
  ): Promise<{ channel: 'email' | 'sms'; sentTo: string }> {
    ...
    await db.update(
      'tbl_sign_recipient',
      { otpHash: codeHash, otpExpiresAt: expiresAt, otpAttempts: 0 },
      'id = ?',
      [recipient.id]
    );
  ...
  async verify(recipientId: string, code: string): Promise<void> {
    ...
    await db.execute('UPDATE tbl_sign_recipient SET otpAttempts = otpAttempts + 1 WHERE id = ?', [
      recipientId,
    ]);
